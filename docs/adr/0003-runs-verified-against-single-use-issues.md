...
a server-recorded `issued_at` alone gives only a *lower* bound on when a Run could
...
length with a floor of ten minutes — and the client must detect expiry *before*
