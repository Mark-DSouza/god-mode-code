...
`eclipse-temurin:21-jre-alpine`, `caddy:2-alpine`, `golang:1.26-alpine`,
...
`infra/terraform/tests/security.tftest.hcl` asserts on every pull request,
because traffic arrives through the outbound-initiated tunnel of ADR-0002. A
...
anything pushed from anywhere else. CI covers all of them, on every commit. So
the hook is not where the search lives — duplicating CI's scanning inside it
would buy coverage that is a strict subset of what already exists, at the cost
of a second pattern list to keep in step. It earns its place by doing what CI
structurally cannot, which is to fire before the tool runs rather than after
the change is made: the difference between a credential that never touched disk
...
invariants are edited and discovering afterwards that they were. An agent told
