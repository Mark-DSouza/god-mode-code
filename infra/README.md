...
The same rule is why the collector reads the journal. The ordinary way to gather
container logs is `loki.source.docker`, which needs the socket — and a collector
holding it is root on the application host for anyone who reaches the collector.
Both containers log to the journal instead, and `terraform test` asserts that
nothing in the deploy document mentions the socket at all.
